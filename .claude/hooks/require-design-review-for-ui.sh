...
if [ "$(merge_stack_from_invocation "$INPUT")" = "azuredevops" ]; then
  ...
  echo "NOTE: require-design-review-for-ui does not apply to Azure DevOps PR #${_AZ_PR:-?} — it reads the diff via the GitHub CLI, which cannot see an AzDO pull request. Review coverage for AzDO PRs is not enforced by this hook." >&2
  exit 0
fi
